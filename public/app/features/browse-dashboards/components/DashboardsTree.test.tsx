...
describe('browse-dashboards DashboardsTree', () => {
  ...
  describe('folder star button', () => {
    ...
    describe('with starsFromAPIServer enabled', () => {
    ...
    describe('with starsFromAPIServer disabled', () => {
      it('does not render a star button for a folder row', async () => {
        renderTree([folder]);

        expect(await screen.findByText(folder.item.title)).toBeInTheDocument();
        expect(screen.queryByTestId(selectors.components.NavToolbar.markAsFavorite)).not.toBeInTheDocument();
      });
    });
  });
